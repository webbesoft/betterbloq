...
// Helper function to calculate delivery date - Now returns ISO string or null
const calculateDeliveryDateISO = (items: CartItem[]): string | null => {
  ...
  return deliveryDate.toISOString(); // Return as ISO string
};
    ...
    (set, get) => ({
      ...
      addItem: (product) => {
        ...
        if (currentVendorId === null || productVendorId === currentVendorId) {
          ...
          console.warn(
            'addItem called with mismatched vendor. This should be handled in the component.'
          );
        }
      },
